...
def parse_args():
    ...
    # Environment
    parser.add_argument("--game", type=str, default='bird',
                        help="the name of the game being played")
    parser.add_argument(
        "--n_actions",
        type=int,
        default=2,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="initial value of epsilon when training")
    parser.add_argument(
        "--final_eps",
        type=float,
        default=0.0001,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether or not in the testing phase")
    parser.add_argument(
        "--double",
        action="store_false",
        default=True,
        help="whether or not to use double DQN")
    parser.add_argument(
        "--dueling",
        action="store_false",
        default=True,
        help="whether or not to use dueling network")
    parser.add_argument(
        "--prioritized",
        action="store_false",
        default=True,
        help="whether or to use prioritized replay buffer")
    parser.add_argument(
        "--prioritized_alpha",
        type=float,
        default=0.6,
...
def playGame():
    ...
    # actions taken: [0, 1] or [1, 0]
    action = tf.placeholder("float", [None, args.n_actions], name="action")
    ...
    if args.prioritized:
        replay_buffer = PrioritizedReplayBuffer(
            args.replay_buffer_size, alpha=args.prioritized_alpha)
    ...
    if checkpoint and checkpoint.model_checkpoint_path:
        ...
        already_trained = int(
            already_trained[already_trained.find('dqn-') + 4:])
    ...
    EpsilonSchedule = LinearSchedule(
        args.explore, args.final_eps, args.initial_eps)
    t = already_trained
    ...
    while "flappy bird" != "angry bird":
        ...
        if t % args.frame_per_action == 0:
            action_index = random.randrange(
                args.n_actions) if random.random() < epsilon else np.argmax(Q_t)
        a_t[action_index] = 1
        ...
        if t > args.observe + already_trained:
            ...
            if args.prioritized:
                experience = replay_buffer.sample(
                    args.batch_size, beta=beta_schedule.value(
                        t - args.observe - already_trained))
                (s_j_batch, a_batch, r_batch, s_j1_batch,
                 done_batch, weights, batch_idxes) = experience
            else:
            ...
            Q_j1_batch = q_target.eval(
                feed_dict={
                    s: s_j1_batch}) if args.double else q_func.eval(
                feed_dict={
                    s: s_j1_batch})
            # -----double---------
