...
def update_target(q_func_vars, target_q_func_vars):
    ...
    for var, var_target in zip(
        sorted(
            q_func_vars, key=lambda v: v.name), sorted(
            target_q_func_vars, key=lambda v: v.name)):
...
def conv2d(
    ...
    with tf.variable_scope(name):
        ...
        filter_shape = [
            filter_size[0], filter_size[1], int(
        ...
        w = tf.get_variable("W",
                            filter_shape,
                            dtype,
                            tf.random_uniform_initializer(-w_bound,
                                                          w_bound),
        ...
        if summary_tag is not None:
            tf.summary.image(summary_tag, tf.transpose(tf.reshape(
                w, [filter_size[0], filter_size[1], -1, 1]), [2, 0, 1, 3]), max_images=10)
...
def dropout(x, pkeep, phase=None, mask=None):
    mask = tf.floor(
        pkeep +
        tf.random_uniform(
            tf.shape(x))) if mask is None else mask
    if phase is None:
        return mask * x
    else:
...
class _MemFriendlyFunction(object):
    ...
    def __call__(self, *inputvals):
        ...
        for i_start in range(0, n, self.batch_size):
            slice_vals = [
                v[i_start:min(i_start + self.batch_size, n)] for v in data_vals]
    ...
    def __call__(self, *args):
        ...
        with tf.variable_scope(self.name, reuse=not self.first_time):
            ...
            if self.first_time:
                self.scope = scope
                print(
                    "(%s) running function for the first time" %
                    (self.name,))
            else:
